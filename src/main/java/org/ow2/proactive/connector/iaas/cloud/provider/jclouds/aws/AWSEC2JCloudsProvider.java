...
public class AWSEC2JCloudsProvider extends JCloudsProvider {
    ...
    // Store the auto-generated security group for each infrastructure, so that it can be removed when delete infrastructure.
    ...
    private void addOptions(Template template, Options options, Infrastructure infrastructure, String region) {
        ...
        if (securityGroupNames != null && !securityGroupNames.isEmpty()) {
            ...
            String sgName = getAutoGeneratedSecurityGroupName(infrastructure.getId());
            if (ports != null) {
                sgName += "_" + UUID.randomUUID();
                String sgDescription = "Auto generated security group to authorize the ports " + Arrays.toString(ports);
                SecurityGroupApi sgApi = getSecurityGroupApi(infrastructure);
                sgApi.createSecurityGroupInRegion(region, sgName, sgDescription);
                for (int port : ports) {
                    sgApi.authorizeSecurityGroupIngressInRegion(region,
                                                                sgName,
                                                                IpProtocol.TCP,
                                                                port,
                                                                port,
                ...
                template.getOptions().as(AWSEC2TemplateOptions.class).securityGroupIds(sgName).inboundPorts(ports);
            }
